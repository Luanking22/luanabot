...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=+5521994819872:+55 (21) 99481-9872\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oii @${num.split('@')[0]}\nBem vindo (a) ao grupo *${mdata.subject}* leia as regras na descrição`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: ' _⏳ Aguarde! ⏳ (se o comando não funcionar tente novamente em ± 3 minutos⏰)_ ',
				success: '✔️Sucesso✔️',
				...
				only: {
					...
					G: '[❗] Este comando só pode ser usado pelo grupo proprietário! ❌',
					B: '[❗] Este comando só pode ser usado pelo Owner! ❌',
					admin: '[❗] Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nOii!\nVocê nn está registrado no banco de dados, tem que se cadastrar pra ter acesso ao menu, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Naruto|18`,
				}
			}
			...
			const ownerNumber = ["5521994819872@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5521994819872@s.whatsapp.net'
                        /******ApiKey Input******/
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, exemplo ${prefix}snowwrite |BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, por exemplo ${prefix}marvellogo  BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*❏ Título* : ${anu.title}\n\n*⏰ O VIDEO ESTA SENDO ENVIADO AGUARDE ± 3 MINUTOS*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘MENCIONANDO TODOS〙✪══\n╠➥'+teks+'╚═〘LUANA BOT〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘MENCIONANDO TODOS〙✪══\n╠➥'+teks+'╚═〘LUANA BOT〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 TRANSMISSÃO DE GRUPO 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'tchau👋', text) // ur cods
					}, 0)
                     break
           ...
           break
				...
				case 'remover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Removendo o lixo 🚮 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Desculpe, o texto é muito longo, máximo de 300 letras', msgType.text, {quoted: mek})
                       ...
                       teks1 = `*[RELATÓRIO]*\nNúmero : @${nomor.split("5521994819872@s.whatsapp.net")[0]}\nmensagem : ${pesan}`
                      var options = {
                         text: teks1,
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde está o texto? exemplo ${prefix}logowolf |Luana`)
					reply(mess.wait)
				...
				case 'logowolf2':
					...
					var teks1 = gh.split("|")[0];
					var teks2 = gh.split("|")[1];
					if (args.length < 1) return reply(`onde está o texto? exemplo ${prefix}logowolf |Lu`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
            ...
            break
					...
					case 'nsfwbobs': 
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ta aí os peitos que vc queria'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
				...
				case 'nsfwass':
					...
					case 'nsfwthighs':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'aaah'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwarmpits':
						...
						try {
							...
							res = await fetchJson(`https://tobz-api.herokuapp.com/nsfwtrap?apikey=BotWeA`, {method: 'get'})
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ta aíi os peitos que vc queria'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}undergocean BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodenboards':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}woodenboards  BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}metalictglow BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'randomkpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: '👁️👄👁️'})
                                        break
                                case 'fml2':
				...
				case 'clone':
					...
					break
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}harrypotter  BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
